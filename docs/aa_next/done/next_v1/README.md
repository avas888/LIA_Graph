...
**Step 8.2.2 — `docs/guide/orchestration.md`.** Two edits.
...
**Step 8.3.1 — static check.** Run `grep -n "LIA_TEMA_FIRST_RETRIEVAL" scripts/dev-launcher.mjs docs/guide/orchestration.md docs/guide/env_guide.md CLAUDE.md frontend/src/app/orchestration/shell.ts`. Verify: launcher line says `= "shadow"`, matrix rows say `shadow`, no residual reference to "`on` (flipped from `shadow` on 2026-04-24)" remains unpaired with its revert clause.
