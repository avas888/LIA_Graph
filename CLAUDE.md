...
| `LIA_CHUNK_QUALITY_ENTITY_FILTER` | `enforce` | Demotes chunks matching named-entity / acta-template / formulario / verbatim-audit-string leak patterns (incl. InnovaLab / Carlos Moreno Pérez) + topic-aware demotion for Concepto-DIAN-depreciación / INC-vehicle / INCRNGO-donations leaks. Promoted shadow→enforce in v25 P8 | `=shadow`/`off` | fix_v23 §3.4 + fix_v25 §3.8 |
...
| `LIA_NORM_KEYED_BOOST` | `enforce` | Detects explicit Res./Decreto/Ley/Acuerdo/Concepto/Sentencia references in the question; injects a polish-prompt directive nudging direct citation in Anclaje Legal + exposes a rerank-time boost helper for matching `norm_id` chunks | `=shadow`/`off` | fix_v25 §3.1 |
| `LIA_CROSS_BORDER_LANE` | `enforce` | Detects foreign-payment context (servicios desde el exterior, royalties, treaty, software cloud) and surfaces canonical ET 408 / 410 / 420 par.3 / 437-2 / 124-1 / 124-2 in the polish prompt so the LLM stops defaulting to domestic ET 392 | `=shadow`/`off` | fix_v25 §3.2 |
| `LIA_MUNICIPAL_TAX_ROUTING` | `enforce` | Detects municipal tax context (Bogotá / Medellín / reteICA / territorialidad) and prepends a canonical local-norm pointer block (Acuerdo 65/2002 + Decreto Distrital 352/2002 for Bogotá) when the corpus lacks SHD sources; polish directive warns against ET-only drift | `=shadow`/`off` | fix_v25 §3.3 |
| `LIA_FRAMEWORK_AWARENESS` | `enforce` | NIIF Pymes vs NIIF Plenas detector + per-framework polish directive; validator rejects `NIIF 16`/`IFRS 16`/`right-of-use` leakage into a NIIF-Pymes answer | `=shadow`/`off` | fix_v25 §3.4 |
| `LIA_COVERAGE_GAP_GATE` | `enforce` | Polish validator rejects "Cobertura pendiente" / "valida el expediente antes" / "no encuentro evidencia para" stub phrases — the LLM must omit unanswered sub-questions silently | `=off` | fix_v25 §3.5 |
| `LIA_DEADLINE_REGISTRY_INJECTION` | `enforce` | Injects per-norm canonical deadlines (RTE March 31; exógena AG-2025 window) + multi-UVT helpers (4 / 27 / 92,000 / 100,000 / 3,300 UVT precomputed per year) into the polish prompt by topic | `=shadow`/`off` | fix_v25 §3.6 |
| `LIA_FALLBACK_NUMERIC_ECHO` | `enforce` | Pre-extracts user-stated peso amounts / UVT counts / percentages from the question and prepends a `**Datos del caso (verbatim del usuario)**` block to fallback output so polish-reject paths cannot remutate user numerics | `=shadow`/`off` | fix_v25 §3.7 |
| `LIA_COUNTERFACTUAL_DETECTOR` | `enforce` | Polish validator rejects polished output containing named persons / company suffixes / large monetary facts absent from BOTH the question and the evidence excerpts; institutional stop-list (DIAN, UGPP, SHD, …) preserved | `=shadow`/`off` | fix_v25 §3.9 |
| `LIA_DOCUMENTO_SOPORTE_LANE` | `enforce` | Detects "documento soporte" + "no obligado a expedir factura" cues; surfaces canonical anchors (Art. 771-2 ET + Resolución DIAN 000167/2021 + Decreto 358/2020 + ET 615/616-1/617) via polish directive + warns against zona-franca drift. Fix-2 for audit Q1 fail | `=shadow`/`off` | fix_v25 P12 |
