...
-- Fix FTS recall: switch from AND-matching (plainto_tsquery) to OR-matching.
-- Both RPCs now accept an optional `fts_query` parameter: a pre-built OR-joined
-- tsquery string produced by Python (_build_fts_or_query).
-- When fts_query IS NOT NULL → to_tsquery('spanish', fts_query) (OR-based).
-- When NULL → plainto_tsquery('spanish', search_query) (backward compat).

-- ============================================================
-- fts_scored_prefilter: drop old signature then recreate
-- ============================================================
DROP FUNCTION IF EXISTS fts_scored_prefilter(TEXT, TEXT[], TEXT, INTEGER, TEXT, TEXT);
...
BEGIN
    -- Build the effective tsquery: OR-based when fts_query provided, AND-based otherwise
...
-- ============================================================
-- hybrid_search: drop old signature then recreate with fts_query
-- ============================================================
DROP FUNCTION IF EXISTS hybrid_search(vector(768), text, text, text, int, int, float, float, text, text);
...
DECLARE
    effective_tsq tsquery;
BEGIN
    -- Build the effective tsquery: OR-based when fts_query provided
    IF fts_query IS NOT NULL AND fts_query <> '' THEN
        effective_tsq := to_tsquery('spanish', fts_query);
    ELSE
