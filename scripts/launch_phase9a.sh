#!/usr/bin/env bash
# Phase 9.A additive reingest launcher (detached).
# Survives terminal close: nohup ignores SIGHUP, disown removes from job table,
# redirects break the tee pipe fragility that killed the prior run.
set -euo pipefail
