...
| Falkor `ArticleNode` count | 9,160 | ≥ 12,500 | ≥ 11,500 | **CORRECTED 2026-04-24 after phase 2c run**: the target was based on the assumption that each parsed-article row produces a new node; the v4 design collapses prose-only rows to one node per source file. Real v6 distinct-MERGE-key count is ~2,440 (1,139 prose + 1,301 numbered), all already present in Falkor from historical runs. New target: **≥ 9,245 ArticleNodes + ≥ 1,100 `whole::` keys + ≥ 2,700 TEMA edges + ≥ 450 HAS_SUBTOPIC**. Post-2c measured: 9,245 / 1,139 / 2,930 / 513 — **all met**. |
...
- ~~`ArticleNode` count ≥ **12,500** (pre-rebuild: 9,160 → Δ ≥ +3,340).~~ **CORRECTED 2026-04-24 (post-phase-2c investigation).** The 12,500 target was miscalculated: it assumed 1 row = 1 node, but `loader.py::_graph_article_key` collapses all prose-only rows from the same source file onto `whole::<source>` (one ArticleNode per source). Real v6 load plan has 2,440 distinct MERGE targets (1,139 prose + 1,301 numbered), all already present in Falkor from historical runs. **Revised gate: ArticleNode ≥ 9,245 AND `whole::` count ≥ 1,100 AND TEMA edges ≥ 2,700.** Post-phase-2c measured: 9,245 / 1,139 / 2,930 — all met.
...
| 2 — Full corpus rebuild | done | 2026-04-24 | 2026-04-24 | (multi-commit: `93698ed..bea9bf8`) | FULL rebuild completed via phase 2a/2b/2c pipeline. Local artifact: 7,883 parsed articles (2,647 NORMATIVA + 1,391 EXPERTOS + 2,218 PRACTICA + 1,627 OTHER), 30,519 typed edges. Cloud Supabase post-sink: 6,730 documents, 19,507 chunks, 144,915 normative_edges. Cloud Falkor post-sink: 9,245 ArticleNodes (1,139 `whole::` prose + 8,106 numbered), 2,930 TEMA, 513 HAS_SUBTOPIC, 92 SubTopicNodes. **Plan's ArticleNode ≥ 12,500 target was a miscalculation** (v4 design collapses prose rows to per-source-file nodes; real v6 load plan has 2,440 distinct keys, all already present). Revised gate met. |
| 2a — Parallel classifier pool | done | 2026-04-24 | 2026-04-24 | `34f658b` |
| 2b — Parallel Supabase sink | done | 2026-04-24 | 2026-04-24 | `602fbb4` | `load_existing_tema` + 4 sink stages reuse the phase-2a pool primitive. Default `--supabase-workers 4`. 9 new tests pass. Motivated by 25-min stall on `load_existing_tema` at t=7m of the first cloud sink attempt on 2026-04-24. |
| 2c — FalkorDB bulk-load hardening | done | 2026-04-24 | 2026-04-24 | `deb71d2` | Fixes the 4-anti-pattern stack from `docs/learnings/ingestion/falkor-bulk-load.md`: (1) `stage_indexes_for_merge_labels` emits CREATE INDEX for every schema label (live-fire: 6 indexes created on cloud in 1.8s — none existed pre-2c); (2) `stage_node_batch` / `stage_edge_batch` emit single UNWIND statements over 500 nodes / 1000 edges; loader groups nodes by kind + edges by (src, edge, dst) so plan caches; (3) per-query `TIMEOUT <ms>` appended to GRAPH.QUERY → server self-aborts + rolls back; (4) `sock.settimeout(query_timeout×2)` before each query + `SO_KEEPALIVE` + TCP_KEEPIDLE/INTVL/CNT at socket open. New env vars: `FALKORDB_QUERY_TIMEOUT_SECONDS` (default 30), `FALKORDB_BATCH_NODES` (500), `FALKORDB_BATCH_EDGES` (1000). 13 new tests + 45 regression tests all green. Smoke landed 3 nodes in 286ms (~110× pre-2c throughput). Full-corpus projection: ~38 statements vs ~28,300 pre-2c. | New `src/lia_graph/ingest_classifier_pool.py` (~130 LOC): thread-safe `TokenBucket` (10s-burst capacity), `classify_documents_parallel` with pre-allocated indexed output + per-future `ThreadPoolExecutor`, decorrelated-jitter retries. `classify_corpus_documents` partitions empty-markdown docs off the pool, merges verdicts back by original index — output order is byte-identical to input across worker counts. **Defaults made persistent: `--classifier-workers` defaults to 8 (env `LIA_INGEST_CLASSIFIER_WORKERS`), `--rate-limit-rpm` default bumped 60 → 300**, propagated to CLI + delta_runtime + delta_worker + UI controller so every ingest path inherits the new defaults. 7/7 new tests pass: output-order invariant, cross-run determinism, global RPM cap under parallelism, per-doc failure isolation, retry-on-transient, empty-input no-op, token-bucket single-thread rate. Web-research-backed design: `executor.map` vs `as_completed` pitfall, Stripe-style idempotency keys, Brooker jitter, O_APPEND atomicity for concurrent events. |
