...
> **Env matrix version**: `v2026-04-21-stv2d` (see `docs/guide/orchestration.md`)
> **Status**: Build V1 Phases 1–3 live; Phase 4 (Railway prod deploy) is the remaining externally-blocked follow-up
...
`npm run dev:production` exits with a Railway notice; the Railway project link is the remaining externally-blocked follow-up from the env fix.
...
### 4. Practical accountant workflows

Refund, correction-firmness, loss-compensation, tax-planning / abuse / simulation shapes are wired; coverage widens per new workflow via `answer_policy.py` blueprints.

### 5. Admin + managed surfaces
