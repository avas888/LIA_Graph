...
concepto cambiario. Smoke H3a=1082 PASS, H3b=537 PASS, H6=5360 PASS;
H1/H2/H4a/H4b/H5 MISS due to YAML keyword patterns (`simple`, `retencion`,
`iva`, `procedimiento`) that don't match the canonical
`concepto.dian.<NUM>` / `oficio.dian.<NUM>.<YEAR>` shape. YAML repair
required to unblock the keyword batches.
...
E6a=1/48 PARTIAL (libro 1 not delivered), E6b=296/64 PASS, E6c=229/64
PASS, J8a=1/160 PARTIAL, J8b=229/160 PASS, J8c=67/160 PARTIAL. Closes
Sprint 2 (briefs 02–05).

**2026-04-28 (PM) Bogotá — brief 04 — ingested 104 rows (DUR 1625 procedimiento).**
Smoke E3a=36/64 PARTIAL, E3b=68/48 PASS. Brief 04 shipped only 1.5.1.* and
similar early subsections; 1.5.6+ not in delivery — would unblock E3a.
...
threshold), E1d=307/64 PASS, E1e=0/64 MISS (sub-libro 1.7.* not in
delivery), E1f=34/48 PARTIAL. Expert noted brief 02 excluded 1.2.4.*
(retención — brief 03) and 1.3.* (IVA — brief 03); after brief 03 lands
those slices may shift. parsed_articles 9427 → 10261; input set 15 070 →
16 192 unique norm_ids.
