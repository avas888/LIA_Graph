...
- **State Notes:**
    ...
    - decision: `src/lia_graph/ingestion/fingerprint.py` landed in Phase 1 (not Phase 2 as originally scoped) because `scripts/ingestion/backfill_doc_fingerprint.py` imports from it. §12.1 subsumption — Phase 2 target-files list narrows to `baseline_snapshot.py` + `test_baseline_snapshot.py` only.
