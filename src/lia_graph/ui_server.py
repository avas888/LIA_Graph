...
for _mod, _names in {
    ...
    "ui_normograma_urls": "_NORMOGRAMA_DIAN_BASE _NORMOGRAMA_MINTIC_BASE _SECRETARIASENADO_ET_BASE _ET_NORMOGRAMA_HOSTS _prefer_normograma_mintic_mirror _prefer_secretariasenado_for_et _load_et_senado_section_map",
    "ui_text_utilities": "_MARKDOWN_BULLET_KV_RE _LOCAL_UPLOAD_URL_RE _NORMALIZED_INGEST_DOC_ID_RE _URL_TRAILING_PUNCT _sb_find_document_row _index_file_signature _load_index_rows_by_doc_id_cached _looks_like_html_document _resolve_knowledge_file _parse_local_upload_url _derive_ingestion_upload_doc_id _resolve_local_upload_artifact _slugify_filename_part _sanitize_url_candidate _clean_markdown_inline _parse_markdown_sections _markdown_section_map _extract_markdown_bullet_metadata _extract_markdown_primary_body_text _reference_doc_catalog _row_is_active_or_canonical _extract_reference_keys_from_citation_payload _reference_base_text_for_request_context _extract_public_reference_text _extract_visible_text_from_html _row_lifecycle_rank _row_curation_rank _extract_named_plain_section_body _SOURCE_BASE_SUMMARY_HEADING_RE _SOURCE_INTERNAL_BOUNDARY_RE _SOURCE_METADATA_LINE_RE",
...
# CLI entry points live in `ui_server_cli`. Re-exported here so
# pyproject.toml `[project.scripts] lia-ui = "lia_graph.ui_server:main"`
# and `python -m lia_graph.ui_server` continue to resolve.
from .ui_server_cli import main, parser, run_server  # noqa: F401, E402
