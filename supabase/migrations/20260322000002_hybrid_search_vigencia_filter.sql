...
-- Migration: Add vigencia filter to hybrid_search RPC
-- Matches the existing filter in fts_scored_prefilter:
--   AND COALESCE(dc.vigencia, '') NOT IN ('derogada', 'proyecto')
-- Prevents derogated/proyecto documents from reaching Python re-rank.

-- Ensure pgvector type is visible to the migration runner
