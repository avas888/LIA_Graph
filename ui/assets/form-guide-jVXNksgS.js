import"./main-BRnL3lSw.js";import{p as W,g as N}from"./client-OE0sHIIg.js";import{p as Z,r as O}from"./format-CYFfBTRg.js";import{i as ee}from"./icons-BZwBwwSI.js";import{c as te}from"./bootstrap-CDL0BdR1.js";import"./index-DE07Z79R.js";import"./authGate-Bb2S6efH.js";function ie(e){return`
  ...
  `}function R(e){const l=String(e||"").trim();return l&&(l.startsWith("/")||/^https?:\/\//i.test(l))?l:""}function re(e){for(const l of e){const g=R(l.url);if(g&&String(l.source_type||"").trim().toLowerCase()==="formulario_oficial_pdf")return{url:g,authority:String(l.authority||"").trim()}}return{url:"",authority:""}}function s(e){return Z(String(e??"").trim())}function j(e){if(typeof e.showModal=="function"){e.open||e.showModal();return}e.setAttribute("open","true")}function F(e){if(typeof e.close=="function"&&e.open){e.close();return}e.removeAttribute("open")}function oe(e){return e.open||e.hasAttribute("open")}function M(e){const l=document.getElementById("view-structured-btn"),g=document.getElementById("view-interactive-btn");if(!l||!g)return;const k=e==="interactive"&&!g.hasAttribute("disabled");g.classList.toggle("view-toggle-active",k),l.classList.toggle("view-toggle-active",!k),g.setAttribute("aria-selected",k?"true":"false"),l.setAttribute("aria-selected",k?"false":"true")}function ne(e){return String(e.label||(e.casilla?`Casilla ${e.casilla}`:"este campo")).trim()}function ae(e){return e.casilla?String(e.casilla):e.label?e.label:"Campo"}function le(e){if(!Array.isArray(e.marker_bbox)||e.marker_bbox.length!==4)return null;const l=e.marker_bbox.map(g=>Number(g));return l.every(g=>Number.isFinite(g))?l:null}function se(e){const l=le(e);return l?{left:l[0]+l[2]/2,top:l[1]+l[3]/2,centered:!0,markerCenterX:l[0]+l[2]/2,markerCenterY:l[1]+l[3]/2}:{left:e.bbox[0],top:e.bbox[1],centered:!1,markerCenterX:null,markerCenterY:null}}function c(e){const l=document.createElement("div");return l.appendChild(document.createTextNode(e)),l.innerHTML}function ce({state:e}){function l(p,d,f,x=""){const w=document.createElement("div");w.className=`form-guide-chat-bubble form-guide-chat-${d} ${x}`.trim();const $=document.createElement("p");$.className="chat-bubble-role",$.textContent=d==="user"?"Tú":"LIA",w.appendChild($);const I=document.createElement("div");I.className="chat-bubble-body",d==="assistant"?O(I,f):I.textContent=s(f),w.appendChild(I),p.appendChild(w),p.scrollTop=p.scrollHeight,e.chatMessages.push({role:d,text:f})}function g(p,d){const f=document.createElement("div");f.className="form-guide-chat-handoff";const x=document.createElement("p");x.className="form-guide-chat-handoff-helper",x.textContent="Esta pregunta se abrirá en el chat general y quedará como borrador para que la continúes allá.";const w=document.createElement("a");w.className="secondary-btn",w.href=d,w.target="_blank",w.rel="noopener noreferrer",w.textContent="Continuar en el chat general",f.append(x,w),p.appendChild(f),p.scrollTop=p.scrollHeight}function k(p){const d=document.createElement("div");return d.className="form-guide-chat-bubble form-guide-chat-thinking",d.setAttribute("aria-live","polite"),d.setAttribute("aria-label","Pensando"),d.innerHTML=`
    ...
    `,p.appendChild(d),p.scrollTop=p.scrollHeight,d}function _(){const p=document.getElementById("form-guide-chat-form"),d=document.getElementById("form-guide-chat-input"),f=document.getElementById("form-guide-chat-log");!p||!d||!f||(p.onsubmit=async x=>{var I;x.preventDefault();const w=d.value.trim();if(!w)return;l(f,"user",w),d.value="",d.disabled=!0;const $=k(f);try{const{data:E}=await W("/api/form-guides/chat",{reference_key:e.currentReferenceKey,profile:e.currentProfile,message:w,selected_field_id:e.selectedFieldId,selected_page:null,active_section:e.selectedSection});if($.remove(),E&&E.ok){const G=E.answer_mode==="pedagogical"?"":"chat-refusal";if(l(f,"assistant",E.answer_markdown,G),E.answer_mode==="out_of_scope_refusal"){const C=R((I=E.grounding)==null?void 0:I.handoff_url);C&&g(f,C)}if(E.suggested_followups&&E.suggested_followups.length>0){const C=E.suggested_followups.map(A=>`<button class="followup-btn" type="button">${c(s(A))}</button>`).join(""),S=document.createElement("div");S.className="chat-followups",S.innerHTML=C,S.addEventListener("click",A=>{const L=A.target.closest(".followup-btn");L&&(d.value=L.textContent||"",p.dispatchEvent(new Event("submit",{bubbles:!0,cancelable:!0})))}),f.appendChild(S)}}else l(f,"assistant","Error procesando tu pregunta. Intenta de nuevo.")}catch{$.remove(),l(f,"assistant","Error de conexión. Intenta de nuevo.")}finally{d.disabled=!1,d.focus()}})}return{bindChatForm:_}}function de(e,l){return e.guide?e.guide:e.reference_key&&Array.isArray(e.available_profiles)?{reference_key:e.reference_key,title:e.title||"",form_version:e.form_version||"",available_profiles:e.available_profiles,supported_views:e.supported_views||[],last_verified_date:e.last_verified_date||"",download_available:!!e.download_available,disclaimer:e.disclaimer||""}:Array.isArray(e.guides)&&e.guides.length>0?e.guides.find(g=>g.reference_key===l)||e.guides[0]:null}function ue({state:e,onContentLoaded:l,onError:g,showLoading:k,showProfileSelector:_}){async function p(){try{const f=await N(`/api/form-guides/content?reference_key=${encodeURIComponent(e.currentReferenceKey)}&profile=${encodeURIComponent(e.currentProfile)}&view=structured`);if(!f.ok){g("No se pudo cargar el contenido de la guía.");return}e.guideSources=f.sources||[],e.guidePageAssets=f.page_assets||[];const x=re(e.guideSources);e.guideOfficialPdfUrl=R(f.official_pdf_url)||x.url,e.guideOfficialPdfAuthority=String(f.official_pdf_authority||x.authority||"").trim(),l(f)}catch{g("Error cargando el contenido de la guía.")}}async function d(){try{const f=await N(`/api/form-guides/catalog?reference_key=${encodeURIComponent(e.currentReferenceKey)}`),x=de(f,e.currentReferenceKey);if(!f.ok||!x){g("Esta guía aún no está disponible.");return}if(x.available_profiles.length>1&&!e.currentProfile){_(x,w=>{e.currentProfile=w,k(),p()});return}!e.currentProfile&&x.available_profiles.length===1&&(e.currentProfile=x.available_profiles[0].profile_id),await p()}catch{g("Error cargando la guía. Intenta de nuevo.")}}return{loadCatalog:d,loadContent:p}}function fe(){return{currentReferenceKey:"",currentProfile:"",selectedFieldId:null,selectedSection:null,chatMessages:[],guideSources:[],guidePageAssets:[],guideOfficialPdfUrl:"",guideOfficialPdfAuthority:"",guideSectionsById:new Map,guideHotspotsById:new Map,guideViewObserver:null}}function ge({state:e}){function l(t){const i=document.createElement("div");return O(i,t),s(i.textContent||"")}function g(t){return l(t).normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/gi," ").trim().toLowerCase()}function k(t,i){const n=g(t.instruction_md||"");if(!n)return!0;const r=g((i==null?void 0:i.title)||""),a=!!(t.casilla&&n.includes(`casilla ${t.casilla}`))&&!!(r&&n.includes(r)),m=n.includes("revise la casilla")||n.includes("verifique la casilla")||n.includes("dentro de la seccion")||n.includes("antes de presentar"),h=!s(t.what_to_review_before_filling||"")&&!s(t.common_errors||"")&&!s(t.warnings||"");return m&&a&&h}function _(t,i){const n=g(t),r=g(i);return!n||!r?!1:n.includes(r)||r.includes(n)}function p(){if(document.querySelectorAll(".guide-hotspot-active").forEach(t=>t.classList.remove("guide-hotspot-active")),document.querySelectorAll(".guide-section-active").forEach(t=>t.classList.remove("guide-section-active")),e.selectedFieldId){const t=document.querySelector(`.guide-hotspot[data-field-id="${e.selectedFieldId}"]`);t==null||t.classList.add("guide-hotspot-active")}if(e.selectedSection){const t=document.querySelector(`.guide-section[data-section-id="${e.selectedSection}"]`);t==null||t.classList.add("guide-section-active")}}function d(){const t=document.getElementById("form-guide-chat-context");if(t){if(e.selectedFieldId){const i=e.guideHotspotsById.get(e.selectedFieldId),n=e.selectedSection?e.guideSectionsById.get(e.selectedSection):void 0,r=i!=null&&i.casilla?`Casilla ${i.casilla} — ${s(i.label)}`:s((i==null?void 0:i.label)||e.selectedFieldId);t.textContent=n?`Contexto: ${r} · ${s(n.title)}`:`Contexto: ${r}`;return}if(e.selectedSection){const i=e.guideSectionsById.get(e.selectedSection);t.textContent=`Contexto: ${s((i==null?void 0:i.title)||e.selectedSection)}`;return}t.textContent=""}}function f(t){const i=document.getElementById("form-guide-field-dialog"),n=document.getElementById("field-dialog-title"),r=document.getElementById("field-dialog-eyebrow"),a=document.getElementById("field-dialog-meta"),m=document.getElementById("field-dialog-summary"),h=document.getElementById("field-dialog-body");if(!i||!n||!r||!a||!m||!h)return;const o=e.guideSectionsById.get(t.section),u=s(ne(t)),b=s(t.official_dian_instruction||""),v=String(t.instruction_md||"").trim();s(v);const T=!!v&&!k(t,o)&&(!b||!_(b,v)),H=s(t.what_to_review_before_filling||"");i.dataset.fieldId=t.field_id,i.dataset.sectionId=t.section,n.textContent=t.casilla?`Casilla ${t.casilla} — ${s(t.label)}`:s(t.label),r.textContent=s((o==null?void 0:o.title)||"Campo guiado"),a.innerHTML=[t.casilla?`<span class="field-dialog-chip">Casilla ${t.casilla}</span>`:"",t.año_gravable?`<span class="field-dialog-chip">AG ${c(t.año_gravable)}</span>`:"",`<span class="field-dialog-chip">Página ${t.page}</span>`,o!=null&&o.title?`<span class="field-dialog-chip field-dialog-chip-soft">${c(s(o.title))}</span>`:""].filter(Boolean).join(""),m.textContent=H,m.hidden=!H;const B=[];if(T){const V=(t.source_ids||[]).map(P=>e.guideSources.find(D=>D.source_id===P)).filter(Boolean).map(P=>P.authority).filter((P,D,J)=>P&&J.indexOf(P)===D),Y=V.length>0?`<span class="field-dialog-source-badge">Fuente: ${c(V.join(", "))}</span>`:"";B.push(`
      ...
      `),!T&&!b&&!t.what_to_review_before_filling&&!t.common_errors&&!t.warnings&&B.push(`
        <article class="field-dialog-card">
      ...
      `),o!=null&&o.title||o!=null&&o.purpose||o!=null&&o.profile_differences){const V=[o!=null&&o.title?`<p><strong>Sección:</strong> ${c(s(o.title))}</p>`:"",o!=null&&o.purpose?`<p><strong>Para qué sirve:</strong> ${c(s(o.purpose))}</p>`:"",o!=null&&o.profile_differences?`<p><strong>Diferencias por perfil:</strong> ${c(s(o.profile_differences))}</p>`:""].filter(Boolean);B.push(`
        <article class="field-dialog-card">
          <p class="guide-section-label">Contexto de la sección</p>
          ${V.join("")}
        </article>
      `)}h.innerHTML=B.join("");const z=h.querySelector('[data-field-dialog-markdown="fallback"]');z&&v&&O(z,v),j(i)}function x(t,i){const n=t.filter(a=>a.section===i).sort((a,m)=>(a.casilla||0)-(m.casilla||0));if(n.length===0)return"";const r=n.map(a=>{const m=a.casilla?`<span class="campo-casilla">Cas. ${a.casilla}</span>`:"",h=c(s(a.label)),o=a.instruction_md?c(l(a.instruction_md).slice(0,160)):"",u=o.length>=160?o+"…":o;return`
          <li class="campo-item" data-campo-field-id="${a.field_id}" role="button" tabindex="0">
            ...
            <span class="campo-open-icon" aria-hidden="true" title="Ver ficha completa">${ee.externalLink}</span>
          </li>
        `}).join("");return`
      <div class="guide-section-block guide-section-campos">
        <p class="guide-section-label">Campos de esta sección (${n.length})</p>
        <ul class="campo-list">${r}</ul>
      </div>
    `}function w(t,i){const n=document.getElementById("structured-sections");n&&(n.innerHTML=t.map(r=>`
      ...
      `).join(""),n.onclick=r=>{const a=r.target.closest("[data-campo-field-id]");if(a){const h=a.dataset.campoFieldId||"",o=e.guideHotspotsById.get(h);o&&(e.selectedFieldId=h,e.selectedSection=o.section,p(),d(),f(o));return}const m=r.target.closest("[data-section-id]");m&&(e.selectedSection=m.dataset.sectionId||null,e.selectedFieldId=null,p(),d())})}function $(t){const i=document.getElementById("interactive-pages");i&&(i.innerHTML=`
    ...
    `)}function I(t,i){const n=new Map;t.forEach(o=>{const u=n.get(o.page)||[];u.push(o),n.set(o.page,u)});const r=document.getElementById("interactive-pages");if(!r)return;const a=new Map;i.forEach(o=>{a.set(o.page,o)});const m=(n.size>0?Array.from(n.keys()):Array.from(a.keys())).sort((o,u)=>o-u);r.innerHTML=m.map(o=>{const u=a.get(o),b=n.get(o)||[];return`
        ...
        `}).join("");const h=Array.from(n.values()).flat();r.onclick=o=>{const u=o.target.closest("[data-field-id]");if(!u)return;const b=u.dataset.fieldId||"",v=h.find(H=>H.field_id===b);if(!v)return;const y=document.getElementById("form-guide-field-dialog"),T=!!(y&&y.dataset.fieldId===b&&oe(y));if(e.selectedFieldId=b,e.selectedSection=v.section,p(),d(),T&&y){F(y);return}f(v)}}function E(){var m;const t=document.getElementById("view-structured-btn"),i=document.getElementById("view-interactive-btn"),n=document.getElementById("form-guide-structured-view"),r=document.getElementById("form-guide-interactive-view"),a=document.querySelector(".form-guide-main");!t||!i||!n||!r||(t.onclick=()=>{M("structured"),n.scrollIntoView({block:"start",behavior:"smooth"})},i.onclick=()=>{i.hasAttribute("disabled")||(M("interactive"),r.scrollIntoView({block:"start",behavior:"smooth"}))},(m=e.guideViewObserver)==null||m.disconnect(),typeof IntersectionObserver=="function"&&(e.guideViewObserver=new IntersectionObserver(h=>{const u=h.filter(b=>b.isIntersecting).sort((b,v)=>v.intersectionRatio-b.intersectionRatio)[0];if(u){if(u.target===r&&!i.hasAttribute("disabled")){M("interactive");return}u.target===n&&M("structured")}},{root:a,threshold:[.35,.6,.85],rootMargin:"-12% 0px -45% 0px"}),e.guideViewObserver.observe(r),e.guideViewObserver.observe(n)))}function G(){const t=document.getElementById("form-guide-field-dialog"),i=document.getElementById("close-field-dialog");t&&(i&&(i.onclick=()=>F(t)),t.onclick=n=>{n.defaultPrevented||F(t)})}function C(){const t=document.getElementById("form-guide-sources-btn"),i=document.getElementById("form-guide-sources-dialog"),n=document.getElementById("close-sources-dialog"),r=document.getElementById("sources-list");!t||!i||!r||(t.onclick=()=>{r.innerHTML=e.guideSources.map(a=>`
          <div class="source-card ${a.is_primary?"source-primary":"source-secondary"}">
            <p class="source-title">${c(s(a.title))}</p>
            <p class="source-authority">${c(s(a.authority))} · ${a.is_primary?"Fuente primaria":"Fuente secundaria"}</p>
            ${a.url?`<a href="${c(a.url)}" target="_blank" rel="noopener noreferrer" class="source-link">Ver fuente</a>`:""}
            ${a.notes?`<p class="source-notes">${c(s(a.notes))}</p>`:""}
            <p class="source-checked">Verificada: ${c(a.last_checked_date)}</p>
          </div>
        `).join(""),j(i)},n&&(n.onclick=()=>F(i)),i.onclick=a=>{a.target===i&&F(i)})}function S(){const t=document.getElementById("form-guide-pdf-btn");t&&(t.disabled=!e.guideOfficialPdfUrl,t.title=e.guideOfficialPdfUrl?e.guideOfficialPdfAuthority?`Abre el PDF oficial publicado por ${e.guideOfficialPdfAuthority}`:"Abre el PDF oficial del formulario.":"Este formulario aún no tiene un PDF oficial publicado en la guía.",t.onclick=()=>{e.guideOfficialPdfUrl&&window.open(e.guideOfficialPdfUrl,"_blank","noopener,noreferrer")})}function A(){const t=document.getElementById("mobile-show-graphic-btn"),i=document.getElementById("form-guide-interactive-view");!t||!i||t.addEventListener("click",()=>{i.style.display="",t.hidden=!0,i.scrollIntoView({block:"start",behavior:"smooth"})})}function L(){const t=document.querySelector(".form-guide-hamburger"),i=document.querySelector(".form-guide-mobile-menu");if(!t||!i)return;t.addEventListener("click",()=>{i.hidden=!i.hidden}),document.addEventListener("click",h=>{!i.hidden&&!i.contains(h.target)&&h.target!==t&&!t.contains(h.target)&&(i.hidden=!0)});const n=document.getElementById("form-guide-sources-btn-mobile"),r=document.getElementById("form-guide-sources-btn");n&&r&&n.addEventListener("click",()=>{i.hidden=!0,r.click()});const a=document.getElementById("form-guide-pdf-btn-mobile"),m=document.getElementById("form-guide-pdf-btn");a&&m&&a.addEventListener("click",()=>{i.hidden=!0,m.click()})}function U(){const t=document.getElementById("form-guide-loading"),i=document.getElementById("form-guide-profile-selector"),n=document.getElementById("form-guide-error"),r=document.getElementById("form-guide-content");t&&(t.hidden=!1),i&&(i.hidden=!0),n&&(n.hidden=!0),r&&(r.hidden=!0)}function K(t,i){const n=document.getElementById("form-guide-loading"),r=document.getElementById("form-guide-profile-selector"),a=document.getElementById("profile-options");n&&(n.hidden=!0),!(!r||!a)&&(r.hidden=!1,a.innerHTML=t.available_profiles.map(m=>`
          <button class="profile-option-btn" type="button" data-profile="${m.profile_id}">
            <strong>${s(m.profile_label)}</strong>
          </button>
        `).join(""),a.onclick=m=>{const h=m.target.closest("[data-profile]");h&&(r.hidden=!0,i(h.dataset.profile||""))})}function Q(t){const i=document.getElementById("form-guide-loading"),n=document.getElementById("form-guide-error"),r=document.getElementById("form-guide-error-message");i&&(i.hidden=!0),n&&(n.hidden=!1),r&&(r.textContent=s(t))}function X(t){const i=document.getElementById("form-guide-loading"),n=document.getElementById("form-guide-content"),r=document.getElementById("form-guide-disclaimer");e.guideSectionsById=new Map(t.structured_sections.map(y=>[y.section_id,y])),e.guideHotspotsById=new Map(t.interactive_map.map(y=>[y.field_id,y])),i&&(i.hidden=!0),n&&(n.hidden=!1);const a=document.getElementById("form-guide-title"),m=document.getElementById("form-guide-profile"),h=document.getElementById("form-guide-version"),o=document.getElementById("form-guide-verified");a&&(a.textContent=s(t.manifest.title)),m&&(m.textContent=s(t.manifest.profile_label)),h&&(h.textContent=s(t.manifest.form_version)),o&&(o.textContent=`Última verificación: ${t.manifest.last_verified_date}`),r&&t.disclaimer&&(r.textContent=s(t.disclaimer),r.hidden=!1),w(t.structured_sections,t.interactive_map);const u=document.getElementById("view-interactive-btn"),b=t.interactive_map.length>0&&e.guidePageAssets.length>0;!b&&u?(u.setAttribute("disabled","true"),u.title="Guía gráfica no disponible para esta guía",$("La guía textual sigue disponible mientras se publica el mapa gráfico certificado.")):u?(u.removeAttribute("disabled"),u.title="",I(t.interactive_map,e.guidePageAssets)):b&&I(t.interactive_map,e.guidePageAssets),E(),G(),C(),S(),L(),A();const v=window.matchMedia("(max-width: 640px)").matches;M(v?"structured":b?"interactive":"structured"),p(),d()}return{renderGuide:X,showError:Q,showLoading:U,showProfileSelector:K}}function me(e,l){const g=new URLSearchParams(window.location.search),k=fe();k.currentReferenceKey=g.get("reference_key")||"",k.currentProfile=g.get("profile")||"";const _=ge({state:k}),p=ce({state:k}),d=ue({state:k,onContentLoaded:_.renderGuide,onError:_.showError,showLoading:_.showLoading,showProfileSelector:_.showProfileSelector});if(!k.currentReferenceKey){_.showError("No se especificó un formulario.");return}p.bindChatForm(),d.loadCatalog()}const q=te({missingRootMessage:"Missing #app root for form-guide page."});q.setTitle(q.i18n.t("app.title.formGuide")||"LIA - Guia de Formulario");q.mountShell(ie());me();
