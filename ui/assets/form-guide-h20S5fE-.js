import"./main-DICslklM.js";import{p as Q,g as N}from"./client-OE0sHIIg.js";import{p as W,r as V}from"./format-CYFfBTRg.js";import{b as Z}from"./bootstrap-CDL0BdR1.js";import"./index-DE07Z79R.js";import"./authGate-Bb2S6efH.js";function ee(e){return`
  ...
  `}function R(e){const l=String(e||"").trim();return l&&(l.startsWith("/")||/^https?:\/\//i.test(l))?l:""}function te(e){for(const l of e){const g=R(l.url);if(g&&String(l.source_type||"").trim().toLowerCase()==="formulario_oficial_pdf")return{url:g,authority:String(l.authority||"").trim()}}return{url:"",authority:""}}function c(e){return W(String(e??"").trim())}function U(e){if(typeof e.showModal=="function"){e.open||e.showModal();return}e.setAttribute("open","true")}function F(e){if(typeof e.close=="function"&&e.open){e.close();return}e.removeAttribute("open")}function ie(e){return e.open||e.hasAttribute("open")}function L(e){const l=document.getElementById("view-structured-btn"),g=document.getElementById("view-interactive-btn");if(!l||!g)return;const E=e==="interactive"&&!g.hasAttribute("disabled");g.classList.toggle("view-toggle-active",E),l.classList.toggle("view-toggle-active",!E),g.setAttribute("aria-selected",E?"true":"false"),l.setAttribute("aria-selected",E?"false":"true")}function ne(e){return String(e.label||(e.casilla?`Casilla ${e.casilla}`:"este campo")).trim()}function oe(e){return e.casilla?String(e.casilla):e.label?e.label:"Campo"}function re(e){if(!Array.isArray(e.marker_bbox)||e.marker_bbox.length!==4)return null;const l=e.marker_bbox.map(g=>Number(g));return l.every(g=>Number.isFinite(g))?l:null}function ae(e){const l=re(e);return l?{left:l[0]+l[2]/2,top:l[1]+l[3]/2,centered:!0,markerCenterX:l[0]+l[2]/2,markerCenterY:l[1]+l[3]/2}:{left:e.bbox[0],top:e.bbox[1],centered:!1,markerCenterX:null,markerCenterY:null}}function s(e){const l=document.createElement("div");return l.appendChild(document.createTextNode(e)),l.innerHTML}function le({state:e}){function l(p,d,f,_=""){const w=document.createElement("div");w.className=`form-guide-chat-bubble form-guide-chat-${d} ${_}`.trim();const B=document.createElement("p");B.className="chat-bubble-role",B.textContent=d==="user"?"Tú":"LIA",w.appendChild(B);const $=document.createElement("div");$.className="chat-bubble-body",d==="assistant"?V($,f):$.textContent=c(f),w.appendChild($),p.appendChild(w),p.scrollTop=p.scrollHeight,e.chatMessages.push({role:d,text:f})}function g(p,d){const f=document.createElement("div");f.className="form-guide-chat-handoff";const _=document.createElement("p");_.className="form-guide-chat-handoff-helper",_.textContent="Esta pregunta se abrirá en el chat general y quedará como borrador para que la continúes allá.";const w=document.createElement("a");w.className="secondary-btn",w.href=d,w.target="_blank",w.rel="noopener noreferrer",w.textContent="Continuar en el chat general",f.append(_,w),p.appendChild(f),p.scrollTop=p.scrollHeight}function E(p){const d=document.createElement("div");return d.className="form-guide-chat-bubble form-guide-chat-thinking",d.setAttribute("aria-live","polite"),d.setAttribute("aria-label","Pensando"),d.innerHTML=`
    ...
    `,p.appendChild(d),p.scrollTop=p.scrollHeight,d}function I(){const p=document.getElementById("form-guide-chat-form"),d=document.getElementById("form-guide-chat-input"),f=document.getElementById("form-guide-chat-log");!p||!d||!f||(p.onsubmit=async _=>{var $;_.preventDefault();const w=d.value.trim();if(!w)return;l(f,"user",w),d.value="",d.disabled=!0;const B=E(f);try{const{data:k}=await Q("/api/form-guides/chat",{reference_key:e.currentReferenceKey,profile:e.currentProfile,message:w,selected_field_id:e.selectedFieldId,selected_page:null,active_section:e.selectedSection});if(B.remove(),k&&k.ok){const G=k.answer_mode==="pedagogical"?"":"chat-refusal";if(l(f,"assistant",k.answer_markdown,G),k.answer_mode==="out_of_scope_refusal"){const S=R(($=k.grounding)==null?void 0:$.handoff_url);S&&g(f,S)}if(k.suggested_followups&&k.suggested_followups.length>0){const S=k.suggested_followups.map(P=>`<button class="followup-btn" type="button">${s(c(P))}</button>`).join(""),x=document.createElement("div");x.className="chat-followups",x.innerHTML=S,x.addEventListener("click",P=>{const M=P.target.closest(".followup-btn");M&&(d.value=M.textContent||"",p.dispatchEvent(new Event("submit",{bubbles:!0,cancelable:!0})))}),f.appendChild(x)}}else l(f,"assistant","Error procesando tu pregunta. Intenta de nuevo.")}catch{B.remove(),l(f,"assistant","Error de conexión. Intenta de nuevo.")}finally{d.disabled=!1,d.focus()}})}return{bindChatForm:I}}function ce(e,l){return e.guide?e.guide:e.reference_key&&Array.isArray(e.available_profiles)?{reference_key:e.reference_key,title:e.title||"",form_version:e.form_version||"",available_profiles:e.available_profiles,supported_views:e.supported_views||[],last_verified_date:e.last_verified_date||"",download_available:!!e.download_available,disclaimer:e.disclaimer||""}:Array.isArray(e.guides)&&e.guides.length>0?e.guides.find(g=>g.reference_key===l)||e.guides[0]:null}function se({state:e,onContentLoaded:l,onError:g,showLoading:E,showProfileSelector:I}){async function p(){try{const f=await N(`/api/form-guides/content?reference_key=${encodeURIComponent(e.currentReferenceKey)}&profile=${encodeURIComponent(e.currentProfile)}&view=structured`);if(!f.ok){g("No se pudo cargar el contenido de la guía.");return}e.guideSources=f.sources||[],e.guidePageAssets=f.page_assets||[];const _=te(e.guideSources);e.guideOfficialPdfUrl=R(f.official_pdf_url)||_.url,e.guideOfficialPdfAuthority=String(f.official_pdf_authority||_.authority||"").trim(),l(f)}catch{g("Error cargando el contenido de la guía.")}}async function d(){try{const f=await N(`/api/form-guides/catalog?reference_key=${encodeURIComponent(e.currentReferenceKey)}`),_=ce(f,e.currentReferenceKey);if(!f.ok||!_){g("Esta guía aún no está disponible.");return}if(_.available_profiles.length>1&&!e.currentProfile){I(_,w=>{e.currentProfile=w,E(),p()});return}!e.currentProfile&&_.available_profiles.length===1&&(e.currentProfile=_.available_profiles[0].profile_id),await p()}catch{g("Error cargando la guía. Intenta de nuevo.")}}return{loadCatalog:d,loadContent:p}}function de(){return{currentReferenceKey:"",currentProfile:"",selectedFieldId:null,selectedSection:null,chatMessages:[],guideSources:[],guidePageAssets:[],guideOfficialPdfUrl:"",guideOfficialPdfAuthority:"",guideSectionsById:new Map,guideHotspotsById:new Map,guideViewObserver:null}}function ue({state:e}){function l(t){const i=document.createElement("div");return V(i,t),c(i.textContent||"")}function g(t){return l(t).normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/gi," ").trim().toLowerCase()}function E(t,i){const r=g(t.instruction_md||"");if(!r)return!0;const n=g((i==null?void 0:i.title)||""),a=!!(t.casilla&&r.includes(`casilla ${t.casilla}`))&&!!(n&&r.includes(n)),m=r.includes("revise la casilla")||r.includes("verifique la casilla")||r.includes("dentro de la seccion")||r.includes("antes de presentar"),b=!c(t.what_to_review_before_filling||"")&&!c(t.common_errors||"")&&!c(t.warnings||"");return m&&a&&b}function I(t,i){const r=g(t),n=g(i);return!r||!n?!1:r.includes(n)||n.includes(r)}function p(){if(document.querySelectorAll(".guide-hotspot-active").forEach(t=>t.classList.remove("guide-hotspot-active")),document.querySelectorAll(".guide-section-active").forEach(t=>t.classList.remove("guide-section-active")),e.selectedFieldId){const t=document.querySelector(`.guide-hotspot[data-field-id="${e.selectedFieldId}"]`);t==null||t.classList.add("guide-hotspot-active")}if(e.selectedSection){const t=document.querySelector(`.guide-section[data-section-id="${e.selectedSection}"]`);t==null||t.classList.add("guide-section-active")}}function d(){const t=document.getElementById("form-guide-chat-context");if(t){if(e.selectedFieldId){const i=e.guideHotspotsById.get(e.selectedFieldId),r=e.selectedSection?e.guideSectionsById.get(e.selectedSection):void 0,n=i!=null&&i.casilla?`Casilla ${i.casilla} — ${c(i.label)}`:c((i==null?void 0:i.label)||e.selectedFieldId);t.textContent=r?`Contexto: ${n} · ${c(r.title)}`:`Contexto: ${n}`;return}if(e.selectedSection){const i=e.guideSectionsById.get(e.selectedSection);t.textContent=`Contexto: ${c((i==null?void 0:i.title)||e.selectedSection)}`;return}t.textContent=""}}function f(t){const i=document.getElementById("form-guide-field-dialog"),r=document.getElementById("field-dialog-title"),n=document.getElementById("field-dialog-eyebrow"),a=document.getElementById("field-dialog-meta"),m=document.getElementById("field-dialog-summary"),b=document.getElementById("field-dialog-body");if(!i||!r||!n||!a||!m||!b)return;const o=e.guideSectionsById.get(t.section),u=c(ne(t)),h=c(t.official_dian_instruction||""),v=String(t.instruction_md||"").trim();c(v);const T=!!v&&!E(t,o)&&(!h||!I(h,v)),D=c(t.what_to_review_before_filling||"");i.dataset.fieldId=t.field_id,i.dataset.sectionId=t.section,r.textContent=t.casilla?`Casilla ${t.casilla} — ${c(t.label)}`:c(t.label),n.textContent=c((o==null?void 0:o.title)||"Campo guiado"),a.innerHTML=[t.casilla?`<span class="field-dialog-chip">Casilla ${t.casilla}</span>`:"",t.año_gravable?`<span class="field-dialog-chip">AG ${s(t.año_gravable)}</span>`:"",`<span class="field-dialog-chip">Página ${t.page}</span>`,o!=null&&o.title?`<span class="field-dialog-chip field-dialog-chip-soft">${s(c(o.title))}</span>`:""].filter(Boolean).join(""),m.textContent=D,m.hidden=!D;const C=[];if(T){const H=(t.source_ids||[]).map(A=>e.guideSources.find(O=>O.source_id===A)).filter(Boolean).map(A=>A.authority).filter((A,O,J)=>A&&J.indexOf(A)===O),Y=H.length>0?`<span class="field-dialog-source-badge">Fuente: ${s(H.join(", "))}</span>`:"";C.push(`
      ...
      `)}b.innerHTML=C.join("");const q=b.querySelector('[data-field-dialog-markdown="fallback"]');q&&v&&V(q,v),U(i)}function _(t,i){const r=t.filter(a=>a.section===i).sort((a,m)=>(a.casilla||0)-(m.casilla||0));if(r.length===0)return"";const n=r.map(a=>{const m=a.casilla?`<span class="campo-casilla">Cas. ${a.casilla}</span>`:"",b=s(c(a.label)),o=a.instruction_md?s(l(a.instruction_md).slice(0,160)):"",u=o.length>=160?o+"…":o;return`
    ...
    `)}function $(t,i){const r=new Map;t.forEach(o=>{const u=r.get(o.page)||[];u.push(o),r.set(o.page,u)});const n=document.getElementById("interactive-pages");if(!n)return;const a=new Map;i.forEach(o=>{a.set(o.page,o)});const m=(r.size>0?Array.from(r.keys()):Array.from(a.keys())).sort((o,u)=>o-u);n.innerHTML=m.map(o=>{const u=a.get(o),h=r.get(o)||[];return`
        ...
        `).join(""),U(i)},r&&(r.onclick=()=>F(i)),i.onclick=a=>{a.target===i&&F(i)})}function x(){const t=document.getElementById("form-guide-pdf-btn");t&&(t.disabled=!e.guideOfficialPdfUrl,t.title=e.guideOfficialPdfUrl?e.guideOfficialPdfAuthority?`Abre el PDF oficial publicado por ${e.guideOfficialPdfAuthority}`:"Abre el PDF oficial del formulario.":"Este formulario aún no tiene un PDF oficial publicado en la guía.",t.onclick=()=>{e.guideOfficialPdfUrl&&window.open(e.guideOfficialPdfUrl,"_blank","noopener,noreferrer")})}function P(){const t=document.getElementById("mobile-show-graphic-btn"),i=document.getElementById("form-guide-interactive-view");!t||!i||t.addEventListener("click",()=>{i.style.display="",t.hidden=!0,i.scrollIntoView({block:"start",behavior:"smooth"})})}function M(){const t=document.querySelector(".form-guide-hamburger"),i=document.querySelector(".form-guide-mobile-menu");if(!t||!i)return;t.addEventListener("click",()=>{i.hidden=!i.hidden}),document.addEventListener("click",b=>{!i.hidden&&!i.contains(b.target)&&b.target!==t&&!t.contains(b.target)&&(i.hidden=!0)});const r=document.getElementById("form-guide-sources-btn-mobile"),n=document.getElementById("form-guide-sources-btn");r&&n&&r.addEventListener("click",()=>{i.hidden=!0,n.click()});const a=document.getElementById("form-guide-pdf-btn-mobile"),m=document.getElementById("form-guide-pdf-btn");a&&m&&a.addEventListener("click",()=>{i.hidden=!0,m.click()})}function j(){const t=document.getElementById("form-guide-loading"),i=document.getElementById("form-guide-profile-selector"),r=document.getElementById("form-guide-error"),n=document.getElementById("form-guide-content");t&&(t.hidden=!1),i&&(i.hidden=!0),r&&(r.hidden=!0),n&&(n.hidden=!0)}function z(t,i){const r=document.getElementById("form-guide-loading"),n=document.getElementById("form-guide-profile-selector"),a=document.getElementById("profile-options");r&&(r.hidden=!0),!(!n||!a)&&(n.hidden=!1,a.innerHTML=t.available_profiles.map(m=>`
          <button class="profile-option-btn" type="button" data-profile="${m.profile_id}">
            <strong>${c(m.profile_label)}</strong>
          </button>
        `).join(""),a.onclick=m=>{const b=m.target.closest("[data-profile]");b&&(n.hidden=!0,i(b.dataset.profile||""))})}function K(t){const i=document.getElementById("form-guide-loading"),r=document.getElementById("form-guide-error"),n=document.getElementById("form-guide-error-message");i&&(i.hidden=!0),r&&(r.hidden=!1),n&&(n.textContent=c(t))}function X(t){const i=document.getElementById("form-guide-loading"),r=document.getElementById("form-guide-content"),n=document.getElementById("form-guide-disclaimer");e.guideSectionsById=new Map(t.structured_sections.map(y=>[y.section_id,y])),e.guideHotspotsById=new Map(t.interactive_map.map(y=>[y.field_id,y])),i&&(i.hidden=!0),r&&(r.hidden=!1);const a=document.getElementById("form-guide-title"),m=document.getElementById("form-guide-profile"),b=document.getElementById("form-guide-version"),o=document.getElementById("form-guide-verified");a&&(a.textContent=c(t.manifest.title)),m&&(m.textContent=c(t.manifest.profile_label)),b&&(b.textContent=c(t.manifest.form_version)),o&&(o.textContent=`Última verificación: ${t.manifest.last_verified_date}`),n&&t.disclaimer&&(n.textContent=c(t.disclaimer),n.hidden=!1),w(t.structured_sections,t.interactive_map);const u=document.getElementById("view-interactive-btn"),h=t.interactive_map.length>0&&e.guidePageAssets.length>0;!h&&u?(u.setAttribute("disabled","true"),u.title="Guía gráfica no disponible para esta guía",B("La guía textual sigue disponible mientras se publica el mapa gráfico certificado.")):u?(u.removeAttribute("disabled"),u.title="",$(t.interactive_map,e.guidePageAssets)):h&&$(t.interactive_map,e.guidePageAssets),k(),G(),S(),x(),M(),P();const v=window.matchMedia("(max-width: 640px)").matches;L(v?"structured":h?"interactive":"structured"),p(),d()}return{renderGuide:X,showError:K,showLoading:j,showProfileSelector:z}}function fe(e,l){const g=new URLSearchParams(window.location.search),E=de();E.currentReferenceKey=g.get("reference_key")||"",E.currentProfile=g.get("profile")||"";const I=ue({state:E}),p=le({state:E}),d=se({state:E,onContentLoaded:I.renderGuide,onError:I.showError,showLoading:I.showLoading,showProfileSelector:I.showProfileSelector});if(!E.currentReferenceKey){I.showError("No se especificó un formulario.");return}p.bindChatForm(),d.loadCatalog()}Z({missingRootMessage:"Missing #app root for form-guide page.",mountApp:fe,renderShell:ee,title:e=>e.t("app.title.formGuide")||"LIA - Guia de Formulario"});
