...
- **Mode semantics.** `shadow` (default) → detect + run A1/A2 + log decisions; **output unchanged**. `enforce` → drop loser bullet lines from the markdown before polish. `off` (or `legacy` alias) → noop.
...
- **Rollback.** `LIA_CONFLICT_RESOLVER_MODE=off` (function noop) or `=shadow` (default; telemetry only).
