...
-- Add knowledge_class column to document_chunks for layer-aware retrieval.
-- Prevents 3× duplicate results when the retriever is called once per layer
-- index but RPCs have no knowledge_class filter.

-- Step 1: Add column + index
ALTER TABLE document_chunks ADD COLUMN IF NOT EXISTS knowledge_class TEXT;
CREATE INDEX IF NOT EXISTS idx_chunks_knowledge_class ON document_chunks (knowledge_class);

-- Step 2: Backfill from parent documents table
UPDATE document_chunks dc
SET knowledge_class = d.knowledge_class
FROM documents d
WHERE dc.doc_id = d.doc_id AND dc.knowledge_class IS NULL;

-- Step 3: Update fts_scored_prefilter RPC with optional knowledge_class filter
-- Drop old overload (4 params) to avoid ambiguous function name on GRANT.
DROP FUNCTION IF EXISTS fts_scored_prefilter(TEXT, TEXT[], TEXT, INTEGER);
...
-- Step 4: Update hybrid_search RPC with optional knowledge_class filter
-- Must DROP first because return type changes (adding knowledge_class column).
DROP FUNCTION IF EXISTS hybrid_search(vector(768), text, text, text, int, int, float, float);
